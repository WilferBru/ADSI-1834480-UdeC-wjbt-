...
<?php 
  ...
  if ($_GET['id_wjbt'] == null or $_GET['id_wjbt'] == '') {
   ...
   try {
    ...
    $sql = "DELETE FROM tipovivienda_wjbt WHERE id_wjbt = ? ";
    $stm = $conn->prepare($sql)->execute(array($id));
    include_once('wjbt_contenido.php');
    echo '<div class="alert alert-danger" role="alert">
            El registro a Sido borrado.
          </div>
          <div class="dataTables_length" id="dataTable_length"><a href="wjbt_tablaTipoVivienda.php" class="btn btn-success b">Volver</a></div> <br>
          ';
    include_once('wjbt_footer.php');

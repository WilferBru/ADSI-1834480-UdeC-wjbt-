...
<?php 
  ...
  try {
    ...
    $sql0 = "SELECT sesionAdmin_wjbt.id_wjbt, genero_wjbt.genero_wjbt FROM sesionAdmin_wjbt INNER JOIN genero_wjbt ON sesionAdmin_wjbt.id_genero_wjbt = genero_wjbt.id_wjbt WHERE sesionAdmin_wjbt.id_wjbt = ?";
    $stm0 = $conn->prepare($sql0);
    $stm0->execute(array($_SESSION['id_wjbt']));
...
<script type="text/javascript">
    function Confirmdelete(){
      var respuesta = confirm('<?=$_SESSION['nombre_wjbt']?> Si ingresaste la contraseña correctamente, tu genero se actualizara y deberas volver a iniciar sesión.');
      if (respuesta == true){
          return true;
      }else{
        return false;
  ...
  <div class="row">
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
      ...
      <h2>Cambiar Genero</h2>
      </div>
      <div class="form-responsive">
        <form action="wjbt_cambioSexAdmin.php" method="post">
        <input type="hidden" name="id" value ="<?=$_SESSION['id_wjbt']?>">
        <div class="row" style="margin-top: 15px;">
            
        <div class="form-group col-md-12">   
        <div class="form-group col-md-7">
         <label for="nombre"><strong>Tu Genero</strong></label>  
